...
bool ConnectBlock_Legacy(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (coins->nTime + Params().StakeMinAge() > block.vtx[1].nTime) // Min age requirement
        ...
        if (!CheckStakeKernelHash(pindex->pprev, block.nBits, coins, prevout, block.vtx[1].nTime, hashProofOfStake))
...
void UnlinkPrunedFiles_Legacy(std::set<int>& setFilesToPrune)
{
...
void FindFilesToPrune_Legacy(std::set<int>& setFilesToPrune, uint64_t nPruneAfterHeight)
{
 ...
 */
bool static FlushStateToDisk_Legacy(CValidationState &state, FlushStateMode mode) {
    const CChainParams& chainparams = Params();
    LOCK2(cs_main, cs_LastBlockFile);
    static int64_t nLastWrite = 0;
    ...
    if (fPruneMode && fCheckForPruning && !fReindex) {
        FindFilesToPrune_Legacy(setFilesToPrune, chainparams.PruneAfterHeight_Legacy());
    ...
    if (fDoFullFlush || fPeriodicWrite) {
        ...
        if (fFlushForPrune)
            UnlinkPrunedFiles_Legacy(setFilesToPrune);
        nLastWrite = nNow;
    }
...
void PruneAndFlush_Legacy() {
    ...
    FlushStateToDisk_Legacy(state, FLUSH_STATE_NONE);
}
...
bool static DisconnectTip_Legacy(CValidationState& state)
{
    ...
    LogPrint("bench", "- Disconnect block: %.2fms\n", (GetTimeMicros() - nStart) * 0.001);
    // Write the chain state to disk, if necessary.
    if (!FlushStateToDisk_Legacy(state, FLUSH_STATE_IF_NEEDED))
        return false;
    // Resurrect mempool transactions from the disconnected block.
...
bool ActivateBestChain_Legacy(CValidationState &state, const CChainParams& chainparams, const CBlock *pblock) {
    ...
    // Write changes periodically to disk, after relay.
    if (!FlushStateToDisk_Legacy(state, FLUSH_STATE_PERIODIC)) {
        return false;
    }
...
CBlockIndex* AddToBlockIndex_Legacy(const CBlockHeader& block)
{
    // Check for duplicate
    uint256 hash = block.GetHash();
    BlockMap::iterator it = mapBlockIndex.find(hash);
...
bool FindBlockPos_Legacy(CValidationState &state, CDiskBlockPos &pos, unsigned int nAddSize, unsigned int nHeight, uint64_t nTime, bool fKnown = false)
{
    LOCK(cs_LastBlockFile);
...
static bool AcceptBlockHeader_Legacy(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex=NULL)
{
    ...
    if (pindex == NULL)
        pindex = AddToBlockIndex_Legacy(block);
...
static bool AcceptBlock_Legacy(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, bool fRequested, CDiskBlockPos* dbp)
{
    ...
    if (fCheckForPruning)
        FlushStateToDisk_Legacy(state, FLUSH_STATE_NONE); // we just allocated more disk space for block files
    return true;
}
...
bool static ProcessMessage_Legacy(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (!vRecv.empty())
            vRecv >> addrFrom >> nNonce;
        if (!vRecv.empty()) {
            vRecv >> LIMITED_STRING(pfrom->strSubVer, MAX_SUBVERSION_LENGTH_LEGACY);
            pfrom->cleanSubVer = SanitizeString(pfrom->strSubVer);
        }
        if (!vRecv.empty()) {
            vRecv >> pfrom->nStartingHeight;
